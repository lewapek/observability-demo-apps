package pl.lewapek.products.service

import pl.lewapek.products.AppError
import pl.lewapek.products.Bootstrap.SttpBackendType
import pl.lewapek.products.ZIOUtils.*
import pl.lewapek.products.config.ForwardingServiceConfig
import pl.lewapek.products.service.ForwardingService.{ForwardRequestInput, ForwardingResponse}
...
class ForwardingService(backend: SttpBackendType, config: ForwardingServiceConfig, tracing: Tracing):
  ...
  private def send(headers: Map[String, String], body: String): IO[AppError, ForwardingResponse] =
    backend
      ...
      .mapBoth(
        AppError.internalServerError("Error sending forwarding request", _),
        _.body.left
          .map(msg => AppError.internalServerError(s"Couldn't read forwarding response: $msg"))
          .flatMap(
            _.as[ForwardingResponse].left
              .map(msg => AppError.internalServerError(s"Couldn't convert json to response model: $msg"))
          )
      )
      .absolve
  end send
