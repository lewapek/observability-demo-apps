...
class InitLoadService(
  ...
  def initLoad(headers: Map[String, String]): IO[AppError, Unit] =
    val productIdsZIO = (1 to 10).map { _ =>
      for
        ...
        product <- productServiceClient.addProduct(headers, input)
      yield product.id
    }
    for
      ...
      ordersZIO = (1 to 5).map { _ =>
        for
          ...
          order <- orderServiceClient.addOrder(headers, OrderInput(chosenProductIds.toList, Some(remark), date))
        yield ()
      }
...
object InitLoadService:
  def initLoad(headers: Map[String, String]): ZIO[InitLoadService, AppError, Unit] =
    ZIO.serviceWithZIO[InitLoadService](_.initLoad(headers))
